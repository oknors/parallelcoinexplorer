<script>
	...
	const determineRoute = async (value) => {
		...
		if (isHexKey(value) ){
			...
			// let addressResponse  = await fetch(`https://jorm.okno.rs/b/parallelcoin/address/${value}`).then(res => res.json().then(json => json["d"]))
			// if (addressResponse) {
			// 	navigate(`addresses/${value}`)
			// 	return
			// }
			// console.log(addressResponse);
			let txResponse  = await fetch(`https://jorm.okno.rs/b/parallelcoin/tx/${value}`).then(res => res.json().then(json => json["d"]))
			if (txResponse) {
			...
			let blockHashResponse  = await fetch(`https://jorm.okno.rs/b/parallelcoin/block/hash/${value}`).then(res => res.json().then(json => json["d"]))
			...
			let blockNumResponse  = await fetch(`https://jorm.okno.rs/b/parallelcoin/block/${value}`).then(res => res.json().then(json => json["d"]))
			if (blockNumResponse) {
